"""
Bot de automação de Enriquecimento de SKU - Sportbay Marketplace.

Lê a planilha "Tags para enriquecimento.xlsx" (aba "Tags"), loga no backoffice
e cria um registro de Enriquecimento de SKU para cada grupo de linhas com o
mesmo par (SKU Sportbay, SKU Seller), adicionando uma Tag Global para cada
linha do grupo antes de salvar.

Uso:
    python enrich_bot.py                 # roda tudo, navegador visível
    python enrich_bot.py --dry-run       # preenche tudo mas NUNCA clica em Salvar
    python enrich_bot.py --headless      # roda sem abrir janela do navegador
    python enrich_bot.py --limit 1       # processa só o primeiro grupo (teste)
    python enrich_bot.py --start-from 3  # pula os 2 primeiros grupos
...
LOGIN_URL = "https://www.marketplace.sportbay.com.br/auth/jwt/login?returnTo=%2Fdashboard%2Fauction%2Flist"
CREATE_URL = "https://www.marketplace.sportbay.com.br/dashboard/sku-enrichment/create"

USERNAME = os.getenv("SPORTBAY_USER")
PASSWORD = os.getenv("SPORTBAY_PASS")
SPREADSHEET_PATH = Path(os.getenv("SPREADSHEET_PATH", PROJECT_DIR / "Tags para enriquecimento.xlsx"))
...
# Para "Categorias predefinidas" a busca precisa ser (quase) exata.
CATEGORY_MATCH_THRESHOLD = 0.92
...
log = logging.getLogger("enrich_bot")
...
def load_tag_rows(path: Path) -> list[TagRow]:
    ...
    for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        sku, sku_seller, curva, categoria, grupo, facet, spec = row[0], row[1], row[2], row[3], row[4], row[5], row[6]
        ...
        rows.append(
            TagRow(
                ...
                curva=str(curva).strip() if curva not in (None, "") else None,
...
def strip_sufixo_a(sku: str) -> str:
    """Remove o sufixo "A" final do código, deixando somente os números.

    Regra restrita ao campo "Código SKU da Sportbay" (coluna A). Só corta quando
    o que sobra é apenas dígitos - "4902A" -> "4902", mas "PAI-4902A" e "4902"
    ficam intactos.
    """
    match = re.fullmatch(r"(\d+)[Aa]", sku.strip())
...
def group_consecutive(rows: list[TagRow]) -> list[SkuGroup]:
    ...
    Só junta linhas ADJACENTES com o mesmo par, conforme a regra do usuário:
    se A/B repetem em sequência, é o mesmo enriquecimento (várias tags),
    e o Salvar só acontece depois da última linha do grupo.
    """
...
# --------------------------------------------------------------------------
# Helpers de UI (MUI Select / MUI Autocomplete)
# --------------------------------------------------------------------------
...
def select_curva_abc(page: Page, curva: str) -> bool:
    texto = curva.upper()
    match = re.search(r"CURVA\s*([ABC])\b", texto) or re.fullmatch(r"\s*([ABC])\s*", texto)
    letra = match.group(1) if match else None
    if letra and select_mui_option_by_data_value(page, "Curva ABC", letra):
        log.info(f'  Curva ABC "{curva}" selecionada.')
        return True
    if select_mui_option_by_text(page, "Curva ABC", curva, threshold=0.6):
        log.info(f'  Curva ABC "{curva}" selecionada (por texto).')
        return True
    return False


def select_categoria(page: Page, categoria: str) -> bool:
    ok, score, chosen = autocomplete_select(
        page,
        label_text="Categorias predefinidas",
        search_text=categoria,
        target_text=categoria,
        threshold=CATEGORY_MATCH_THRESHOLD,
    )
    if ok:
        log.info(f'  Categoria "{categoria}" -> "{chosen}" (score {score:.2f})')
    else:
        log.warning(f'  Categoria "{categoria}" SEM correspondência exata (melhor score {score:.2f}) - pulei')
    return ok
...
def wait_salvar_habilitado(page: Page, timeout_ms: int = DEFAULT_TIMEOUT) -> Locator:
    botao = page.get_by_role("button", name="Salvar Enriquecimento")
...
def process_group(page: Page, group: SkuGroup, grupo_lookup: dict[str, str],
                   idx: int, total: int, dry_run: bool, pendencias: list[dict]) -> bool:
    log.info(f"[{idx}/{total}] SKU Sportbay={group.sku} | SKU Seller={group.sku_seller} "
             f"({len(group.rows)} tag(s))")

    page.goto(CREATE_URL)
    page.wait_for_selector('input[name="sku"]', timeout=DEFAULT_TIMEOUT)

    sku_sportbay = strip_sufixo_a(group.sku)
    if sku_sportbay != group.sku:
        log.info(f'  Código SKU da Sportbay: "{group.sku}" -> "{sku_sportbay}" (sufixo A removido)')

    page.fill('input[name="sku"]', sku_sportbay)
    page.fill('input[name="skuSeller"]', group.sku_seller)

    first = group.rows[0]
    if first.curva:
        if not select_curva_abc(page, first.curva):
            log.warning(f'  Curva ABC "{first.curva}" não encontrada')
    if first.categoria:
        select_categoria(page, first.categoria)
    ...
    try:
        ...
        log.error("  Botão 'Salvar Enriquecimento' não habilitou - screenshot salvo, pulando grupo")
...
def main() -> None:
    ...
    log.info(f"{len(rows)} linha(s) de tag lidas -> {len(groups)} grupo(s) de SKU a processar.")
    ...
    with sync_playwright() as p:
        ...
        for i, group in enumerate(groups, start=start + 1):
            try:
                ok = process_group(page, group, grupo_lookup, i, start + len(groups), args.dry_run, pendencias)
    ...
    log.info(f"Concluído: {sucesso} grupo(s) ok, {falha} grupo(s) com falha.")
    if pendencias:
