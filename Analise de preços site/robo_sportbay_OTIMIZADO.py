...
# --- CONFIGURAÇÕES ---
NOME_FICHEIRO_ENTRADA = 'Base dos Produtos.xlsx'
NOME_FICHEIRO_SAIDA = 'Resultado_Menor_Preco_Sportbay.xlsx'
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('robo_sportbay.log', encoding='utf-8'),
...
def _decidir(api, codigo, termo, tokens, confianca_nome):
    ...
    if confirmados:
        ...
        # resolveu — resolver_produto tenta o próximo (sem sufixo A, ou nome).
        return None
...
def resolver_produto(api, codigo, nome_planilha):
    ...
    # Códigos terminados em 'A' costumam ter a variante sem sufixo no site.
    termos = [codigo]
    if len(codigo) > 1 and codigo[-1].upper() == 'A':
        termos.append(codigo[:-1].strip())
